...
/**
 * Rebuild clusters from a subset of edges (e.g. after a reranker has
 * pruned obviously-related-not-duplicate pairs). Runs union-find over
 * the given edges, then computes cluster metadata (mean similarity,
 * LCS, shared n-grams, preview, crossSection) exactly as the trigram
 * and semantic paths do. Paragraphs not connected by any kept edge
 * drop out; singleton components are omitted.
 */
export function reclusterFromEdges(
  paragraphs: Paragraph[],
  edges: DuplicationEdge[],
): DuplicationAnalysis {
  if (edges.length === 0) return { clusters: [], edges: [] }

  // Union-find over the paragraphs referenced by any edge.
  const parent = new Map<string, string>()
  const seed = (id: string) => { if (!parent.has(id)) parent.set(id, id) }
  for (const e of edges) { seed(e.from); seed(e.to) }
  const find = (x: string): string => {
    let r = x
    while (parent.get(r) !== r) r = parent.get(r)!
    let cur = x
  ...
  for (const e of edges) {
    pairSims.set(pairKey(e.from, e.to), e.similarity)
    union(e.from, e.to)
  }
  ...
  for (const ids of groups.values()) {
    ...
    for (let i = 0; i < ids.length; i++) {
      for (let j = i + 1; j < ids.length; j++) {
        const s = pairSims.get(pairKey(ids[i], ids[j]))
  ...
  const rebuiltEdges: DuplicationEdge[] = []
  for (const e of edges) {
    const cid = clusterByRoot.get(find(e.from))
    if (!cid) continue
    rebuiltEdges.push({ from: e.from, to: e.to, similarity: e.similarity, clusterId: cid })
  }
  return { clusters: out, edges: rebuiltEdges }
}
